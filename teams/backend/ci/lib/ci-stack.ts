...
export class CiStack extends Stack {
  constructor(scope: Construct, id: string, props: CiStackProps) {
    ...
    const s3Source = CodePipelineSource.s3(
      bucket,
      `backend/apps/template-manager/config.zip`,
      {
    ...
    const pipeline = new CodePipeline(this, "synth-sources", {
      pipelineName: `${props.stage}-argocd-apps-team-backend`,
      synth: new pipelines.CodeBuildStep("Synth", {
        ...
        commands: [
          ...
          "cd repo/teams/backend/ci",
          "yarn install",
          "npx cdk synth",
        ],
        primaryOutputDirectory: "repo/teams/backend/ci/cdk.out",
      }),
    });

    const triggerStage = new CiTriggerStage(
      this,
      "trigger-pipeline-backend",
      props
    );
    ...
    const trigger = new pipelines.CodeBuildStep("update-files-and-commit", {
      ...
      commands: [
        ...
        `mkdir repo`,
        `git clone --depth 1 -b ${props.branch} https://github.com/tusharf5/capstone-project-app-of-apps.git repo`,
        `mv config.json repo/teams/backend`,
        `cd repo/teams/backend`,
        `yarn install`,
